...
class OfflineMeetingParticipationServiceTest(TestCase):
    def test_success_must_store_worksheet_id_to_cache_when_new_participate(self):
        """
        channel_id 가 cache에 기록된 적이 없고
        새로운 최초 참여 시도(is_new)라면, cache에 {channel_id: True}를 기록한다
        """
        # given
        ...
        is_exist_worksheet_cache = ExpiringDict(max_len=100, max_age_seconds=60 * 60)
        ...
        service = OfflineMeetingParticipationService(
            ...
            is_exist_worksheet_cache=is_exist_worksheet_cache,
        )
    ...
    def test_success_must_create_worksheet_when_real_first_participation(self):
        ...
        is_exist_worksheet_cache = ExpiringDict(max_len=100, max_age_seconds=60 * 60)
        is_exist_worksheet_cache[channel_id] = True
