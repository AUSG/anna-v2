...
from service.offline_meeting.offline_meeting_participation_service import is_target_emoji
...
def test_false_when_type_not_exist():
    event = _create_event()
    del event['type']
    assert is_target_emoji(event, None) is False
...
def test_false_when_event_is_in_reply_in_thread():
    with patch('service.offline_meeting.offline_meeting_participation_service.is_reply_in_thread') as mock_is_reply_in_thread:
...
def test_true_when_event_is_in_reply_in_thread():
    with patch('service.offline_meeting.offline_meeting_participation_service.is_reply_in_thread') as mock_is_reply_in_thread:
        ...
        event = _create_event()
        assert is_target_emoji(event, None) is True
