...
from service.offline_meeting.offline_meeting_service import is_gogo_emoji_on_first_reply
...
def test_false_when_type_not_exist():
    event = _create_event()
    del event['type']
    assert is_gogo_emoji_on_first_reply(event, None) is False
...
def test_true_when_event_is_in_reply_in_thread():
    with patch('service.offline_meeting.offline_meeting_service.is_reply_in_thread') as mock_is_reply_in_thread:
        ...
        event = _create_event()
        assert is_gogo_emoji_on_first_reply(event, None) is True
