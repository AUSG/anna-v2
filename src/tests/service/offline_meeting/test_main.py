...
def _dummy_event():
    return {
        ...
        "user": "U1234567890",
        "item": {"ts": "1663216298.924399", "channel": "announcement_channel_id"},
    }
...
def test_success():
    with patch(
        "implementation.google_spreadsheet_client.GoogleSpreadsheetClient"
    ) as mock_gs_client, patch(
        "implementation.slack_client.SlackClient"
    ) as mock_slack_client, patch(
        "service.offline_meeting.action_commander.ActionCommander"
    ) as mock_action_commander:
        ...
        sut = OfflineMeetingParticipationService(
            event,
            mock_action_commander,
            mock_slack_client,
            mock_gs_client,
            mock_member_finder,
            mock_worksheet_maker,
