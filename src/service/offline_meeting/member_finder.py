...
class MemberFinder:
    ...
    def find(self, slack_unique_id: str) -> Member:
        ...
        if not member:
            raise RuntimeException(
                f"멤버 정보를 찾지 못했어. (slack_unique_id: {slack_unique_id})"
            )
        elif not self._validate_member_info(member):
            raise RuntimeException(
                f"멤버 정보가 완벽하지 않아요. (slack_unique_id: {slack_unique_id}, member_info: {member})"
            )
        else:
            return member
        ...
        for m in raw_members[1:]:  # [0] == header row
            ...
            try:
                ...
                members[user_id] = Member(
                    kor_name, eng_name, email, phone, school_name_or_company_name
                )
    ...
    def _validate_member_info(self, m: Member):
        if (
            m.kor_name is None
            or m.eng_name is None
            or m.email is None
            or m.phone is None
            or m.school_name_or_company_name is None
            or len(m.kor_name) == 0
            or len(m.eng_name) == 0
            or len(m.email) == 0
            or len(m.phone) != 13
            or len(m.school_name_or_company_name) == 0
        ):
            return False
        else:
            return True
