...
저녁 6시까지 기다리지 않고 지금 바로 확인하려면, 아래처럼 대상을 멘션해서 수동으로 돌려볼 수 있다.

```sh
@ANNA 빅챗 리마인더 테스트 @받을사람
```

"지금 기준 내일" 열리는 빅챗을 실제 발송과 똑같이 찾되, DM 은 멘션한 사람에게만 간다. 신청자 수와 슬랙 계정 매칭 결과도 같이 알려주므로, 발송이 안 될 때 시트 이름 문제인지 이메일 매칭 문제인지 바로 구분할 수 있다. (실수로 신청자 전원에게 나가지 않도록 대상 멘션은 필수다)
